...
class Database:
    ...
    def create_if_non_existant(self, user_id: int, name: str):
        profile: DocumentReference = self.db.collection(str(user_id)).document("profile")
        if check_if_playlist_exists(profile, name):
            return
        playlist_id = str(uuid.uuid4())
    ...
    def get_playlist_doc(self, user_id: int, name: str) -> DocumentReference:
        profile: DocumentReference = self.db.collection(str(user_id)).document("profile")
        playlist_doc: DocumentReference = self.db.collection(str(user_id)).document(check_if_playlist_exists(profile, name))
    ...
    def remove_from_playlist(self, user_id: int, name: str, tracks: List[Track]):
        profile: DocumentReference = self.db.collection(str(user_id)).document("profile")
        playlist_doc: DocumentReference = self.db.collection(str(user_id)).document(check_if_playlist_exists(profile, name))
        if playlist_doc is False:
            raise DatabaseExceptions.PlaylistDoesNotExistException()
        existing_contents = self.get_playlist_contents(user_id, name)
        new_contents = existing_contents
        for track in tracks:
            new_contents = [item for item in new_contents if item["url"] != track.uri]
        playlist_doc.set({"contents": new_contents}, False)

    def raise_if_not_exists(self, user_id: int, name: str) -> str:
        profile: DocumentReference = self.db.collection(str(user_id)).document("profile")
        playlist_id = check_if_playlist_exists(profile, name)
        if playlist_id is False:
            raise DatabaseExceptions.PlaylistDoesNotExistException()
        return playlist_id

    def check_if_exists(self, user_id: int, name: str) -> bool:
        profile: DocumentReference = self.db.collection(str(user_id)).document("profile")
        playlist_id = check_if_playlist_exists(profile, name)
        if playlist_id is not False:
            return playlist_id
        return False
...
class DatabaseExceptions:
    class NoProfileFoundException(Exception):
        def __init__(self):
            ...
            super(DatabaseExceptions.NoProfileFoundException, self).__init__("No profile found for the given user id.")
    ...
    class PlaylistCorruptedException(Exception):
        def __init__(self, name: str, user_id: int):
            ...
            super(DatabaseExceptions.PlaylistCorruptedException, self).__init__(f"The given playlist (uid: {user_id}, name: {name}) seems to be corrupted, i.e. important keys are missing.")
