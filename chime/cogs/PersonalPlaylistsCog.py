...
class PersonalPlaylistsCog(commands.Cog, name="Personal Playlists"):
    ...
    async def add(self, ctx: Context, playlist: str, *, search_term: str = None):
        ...
        await search_song(search_term, ctx, self.bot, success_callback, success_callback_url)
        ...
        if len(tracks_to_add) > 0:
            if x is not False:
                playlist_doc: DocumentReference = self.db.collection(str(ctx.author.id)).document(x)
                if playlist_doc is not None:
                    playlist_doc.update({"contents": firestore.ArrayUnion([{"title": track_to_add.title, "author": track_to_add.author, "data": track_to_add.id} for track_to_add in tracks_to_add])})
                    await ctx.message.add_reaction("<:OK:716230152643674132>")
                else:
                    raise BadRequestException(f"No playlist with the name {playlist} exists!")
            else:
